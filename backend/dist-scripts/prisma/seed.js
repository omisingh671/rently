...
async function main() {
    ...
    if (emailOwner &&
        ...
        throw new Error(`Cannot seed SUPER_ADMIN with email ${credentials.superAdmin.email}: email is already used by another user.`);
    }
    ...
    await Promise.all(seededAmenities.map((amenity) => prisma.amenity.upsert({
