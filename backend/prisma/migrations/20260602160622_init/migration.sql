...
CREATE TABLE `properties` (
    ...
    UNIQUE INDEX `properties_tenantId_slug_key`(`tenantId`, `slug`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `property_booking_policies` (
    ...
    `checkInTime` VARCHAR(191) NOT NULL DEFAULT '12:00',
...
CREATE TABLE `payments` (
    ...
    `method` ENUM('CASH', 'UPI_MANUAL', 'BANK_TRANSFER', 'CARD_POS', 'MANUAL', 'ONLINE_GATEWAY') NOT NULL DEFAULT 'MANUAL',
...
CREATE TABLE `payment_refunds` (
    ...
    INDEX `payment_refunds_refundRequestId_idx`(`refundRequestId`),
...
CREATE TABLE `taxes` (
    ...
    `category` ENUM('GENERIC', 'GST') NOT NULL DEFAULT 'GENERIC',
    `scope` ENUM('BOOKING', 'ACCOMMODATION') NOT NULL DEFAULT 'BOOKING',
    `targetType` ENUM('ALL', 'ROOM', 'UNIT') NOT NULL DEFAULT 'ALL',
    `calculationMode` ENUM('FLAT', 'SLAB_PER_ITEM_NIGHTLY_TARIFF') NOT NULL DEFAULT 'FLAT',
    `discountTreatment` ENUM('BEFORE_TAX') NOT NULL DEFAULT 'BEFORE_TAX',
    `minTariff` DECIMAL(65, 30) NULL,
    `maxTariff` DECIMAL(65, 30) NULL,
    ...
    INDEX `taxes_propertyId_category_scope_targetType_isActive_idx`(`propertyId`, `category`, `scope`, `targetType`, `isActive`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `billing_settings` ADD CONSTRAINT `billing_settings_propertyId_fkey` FOREIGN KEY (`propertyId`) REFERENCES `properties`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `billing_documents` ADD CONSTRAINT `billing_documents_bookingId_fkey` FOREIGN KEY (`bookingId`) REFERENCES `bookings`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `billing_documents` ADD CONSTRAINT `billing_documents_paymentId_fkey` FOREIGN KEY (`paymentId`) REFERENCES `payments`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `billing_documents` ADD CONSTRAINT `billing_documents_propertyId_fkey` FOREIGN KEY (`propertyId`) REFERENCES `properties`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `billing_documents` ADD CONSTRAINT `billing_documents_tenantId_fkey` FOREIGN KEY (`tenantId`) REFERENCES `tenants`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
