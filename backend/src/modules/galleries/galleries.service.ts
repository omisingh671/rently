...
import * as repo from "./galleries.repository.js";
import {
  getActor,
  getPropertyScope,
...
import type { GalleryDTO } from "./galleries.dto.js";
...
const mapGallery = (gallery: repo.GalleryRecord): GalleryDTO => ({
  id: gallery.id,
...
export const createGallery = async (
  ...
  if (resolvedRoomId) {
    ...
    if (resolvedUnitId && room.unitId !== resolvedUnitId) {
      throw new HttpError(
        400,
        "INVALID_ROOM_UNIT",
        "Room does not belong to the selected unit",
      );
    }
  ...
  let gallery: repo.GalleryRecord;
  try {
